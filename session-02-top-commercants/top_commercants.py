...
with open ("revolut.csv", "r", encoding="utf-8") as fichier:
    ...
    for ligne in lecteur:
        ...
        if type_transaction == "Paiement par carte":
            ...
            if description not in depenses_par_commercant:
                depenses_par_commercant[description] = 0
            depenses_par_commercant[description] = depenses_par_commercant[description] + montant_positif
...
pourcentage_de_depense = total_depenses_cooltra / total_depenses
...
print("pourcentage de depense :", round(pourcentage_de_depense * 100, 2), "%")
top_20 = sorted(depenses_par_commercant.items(), key=lambda x: x[1], reverse=True)[:20]

print("\n--- TOP 20 COMMERCANTS ---")
for i, (nom, total) in enumerate(top_20):
    print(f"{i+1}. {nom} : {round(total, 2)} €")